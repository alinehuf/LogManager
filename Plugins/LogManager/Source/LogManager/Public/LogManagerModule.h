...
public:
    ...
    /**
     * Requests the logging library to begin its shutdown process.
     * This is asynchronous; use IsShutdownFinished() to check completion.
     */
    void RequestShutdown();
    /**
     * Checks whether the shutdown process of the logging library has completed.
     * This is useful to ensure that all pending log entries have been flushed
     * and the library is safe to unload.
     *
     * @return True if shutdown is finished, false if still in progress.
	 */
    ...
    private:
        TUniquePtr<LogManager> logManager; // Unique pointer to the LogManager instance, managing the logging operations
};
